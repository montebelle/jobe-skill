...
## Input
`$ARGUMENTS` after "apply": a company-role slug (matching a `${WORKSPACE}/reports/{slug}/` folder) or a URL.
Flags: `--paste` (skip the browser, fall back to paste-ready blocks — see apply-assisted.md, for login-walled forms) · **headless is the default and the preferred mode for real submissions** (no visible window, and empirically cleaner under Camoufox — a real display leaks signals that conflict with the spoofed fingerprint and trip security/spam checks). Pass `--headful` only when you need to watch a run or clear a visible wall; headful can *trigger* the checks headless avoids. · **Ashby auto-enables warm mode** (persistent profile + Google warm-up + slow typing + pre-submit dwell) to survive its reCAPTCHA v3 spam gate — run Ashby jobs ONE AT A TIME and never retry a `submit-blocked-spam` tenant (see apply-all.md). `--warm` / `--no-warm` override.

## Prerequisites
1. A completed evaluation in `${WORKSPACE}/reports/{slug}/` with a resume JSON + DOCX (and ideally a cover letter DOCX). If none exists, run `/jobe [url]` (evaluate) first.
2. `camoufox-js` + `playwright-core` installed and the binary fetched (`npx camoufox-js fetch`). One-time.
3. `modes/_profile.md` contact info, and (optionally) `${WORKSPACE}/data/apply-profile.json` for work authorization + EEO choices — the harness pulls contact + answers from the report JSON + that profile via `lib/apply/answers.js`.

## Step 1: Launch the harness (background, holds the form open)
```bash
node scripts/camoufox-apply.js run {slug} [--url {override}] [--headless] &
```
Run it in the **background** — the process keeps the browser alive across the glance and the email-confirm step. It writes status to `${WORKSPACE}/signals/apply/{slug}/state.json` and waits for the agent to write `${WORKSPACE}/signals/apply/{slug}/control.json`.
...
Poll `${WORKSPACE}/signals/apply/{slug}/state.json` until `phase === "filled"`. Read `filled`, `unfilled`, and `questions`.

For each entry in `questions[]`, WRITE a grounded answer (this is the agent's job — never let the harness guess):
- Pull evidence from `${WORKSPACE}/reports/{slug}/resume-*.json` (summary, experience bullets) and the analysis markdown.
- Follow `_shared.md` anti-fabrication + the user's `${WORKSPACE}/reference.md` evidence. Real metrics only. No internal project names. **ATS-clean: no `< > [ ] { } " \`** (the harness types raw text — it does NOT run `lib/normalize.js`).
...
```bash
node -e "require('fs').writeFileSync('${WORKSPACE}/signals/apply/{slug}/control.json', JSON.stringify({action:'answers', answers:[{label:'<question label substring>', text:'<answer>'}]}))"
```
...
Read `${WORKSPACE}/signals/apply/{slug}/preview.png` and present to the user:
```
...
Screenshot: ${WORKSPACE}/signals/apply/{slug}/preview.png
```
...
## Step 4: Submit
```bash
node -e "require('fs').writeFileSync('${WORKSPACE}/signals/apply/{slug}/control.json', JSON.stringify({action:'submit'}))"
```
...
3. Inject it into the SAME stealth session:
   ```bash
   node -e "require('fs').writeFileSync('${WORKSPACE}/signals/apply/{slug}/control.json', JSON.stringify({action:'confirm', link:'<url>'}))"
   ```
...
4. Append a follow-up to `${WORKSPACE}/data/followups.md` (next follow-up = +7 days).
...
- EEO / demographic questions: self-identification is OPT-IN and the user's own choice. By default the harness **declines** every demographic question. A user who wants to self-identify sets `eeoSelfIdentify: true` and fills `eeoValues` (with their OWN gender / race / veteran / disability / etc. answers) in `${WORKSPACE}/data/apply-profile.json`; the harness then matches those against each form's rendered options via `lib/apply/answers.js` `eeoValues` + `lib/apply/filler.js`. Any question with no value on file always declines.
