...
async function braveFetch(query, apiKey) {
...
function buildSiteQueries(queries, filters) {
  ...
  const location = filters.remoteOnly ? 'Remote' : '';
  const out = [];
  ...
  for (const domain of ATS_DOMAINS) {
    ...
    for (const role of ROLE_PATTERNS) {
      out.push({ query: `${site} ${role}${remoteHint}${afterClause}`, domain, location });
    }
  }
  ...
  for (const domain of [...COMPANY_SITES, ...STARTUP_SITES]) {
    const site = siteClauseFor(domain);
    out.push({ query: `${site} (${orClause})${remoteHint}${afterClause}`, domain, location });
  }
...
async function discover(ctx) {
  ...
  logger.info(`[${ID}] running ${siteQueries.length} Brave site-scoped queries`);
  ...
  for (const { query, domain, location } of siteQueries) {
    queryIdx++;
    try {
      const data = await braveFetch(query, apiKey);
