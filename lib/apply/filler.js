...
async function setText(page, idx, value) {
  const loc = page.locator(`[data-cfx="${idx}"]`);
  const v = String(value);
  await loc.fill('');
  await loc.type(isUrlish(v) ? v : normalize(v), { delay: 18 }); // small delay = human-ish; prose is hyphen-stripped
}
  ...
  for (const f of fields) {
    ...
    if (/salary|compensation|expected (base|pay|comp)|pay expectation|desired (salary|comp|pay)|comp expectation/.test(ql) && answers.salaryExpectation) {
      try {
        ...
        else { await setText(page, f.idx, answers.salaryExpectation); filled.push({ label: f.label, value: answers.salaryExpectation }); continue; }
    ...
    if (/how did you hear|how were you referred|referral source|how you heard|where did you (hear|find)/.test(ql)) {
      try {
        ...
        else { await setText(page, f.idx, answers.howHeard); filled.push({ label: f.label, value: answers.howHeard }); continue; }
...
module.exports = {
  detectAts, fillForm, fillQuestions, clickApplyIfPresent, submitLocator, SCAN_FN,
};
