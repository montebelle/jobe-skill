 ...
 * industry-wide; the candidate's personal experience converts at a different rate.
 * For ghost-scoring purposes, what matters is the relative comparison
 * between companies: a company where the candidate has applied 5 times with 0
...
function computeCompanyStats() {
  ...
  for (const [slug, stats] of byCompany) {
    out[slug] = {
      ...stats,
      hiresPerPosting: stats.applications >= 3 ? stats.responses / stats.applications : null,
    };
  }
  return out;
}
