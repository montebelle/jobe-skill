...
const QUERY_TAILS = [
  '"machine learning" after:DATE',
  '"data scientist" remote after:DATE',
  '"senior" remote after:DATE',
];
...
async function harvestSlugs({ apiKey, maxAgeDays = 30, logger = console } = {}) {
  ...
  const dateStr = dateCutoff(maxAgeDays);
  ...
  for (const dom of HARVEST_DOMAINS) {
    ...
    for (const site of dom.sites) {
      for (const tail of QUERY_TAILS) {
        const query = `site:${site} ${tail.replace('DATE', dateStr)}`;
        queriesRun++;
        try {
