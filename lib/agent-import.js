...
const { buildProfile, quickScore, fullScore, isRoleMatch } = require('./rank');
...
async function importAgentDiscoveries(date = null) {
  ...
  // Filter + score against the user's profile (same as the main pipeline), so
  // the WebSearch fallback path is gated and ranked by the user's target roles,
  // not left permissive + profile-blind.
  const profile = buildProfile({ root: ROOT });
  const { kept, rej } = applyAgentFilters(normalized, profile);
  for (const p of kept) quickScore(p, { profile });
