...
// Full US state names (lowercased) so "Austin, Texas" reads as US-onsite while
// "Pune, Maharashtra" / "Toronto, Ontario" read as FOREIGN onsite.
...
// Country/region tokens that are unambiguously non-US. NOTE: do NOT add bare
// city names that collide with US towns (Paris TX, Berlin CT/NH, Munich ND) —
// those must be disambiguated by a country/state suffix, not a naked city
// match. The added tokens below (cyprus/gibraltar/luxembourg/malta/andorra/
// latam/latin america/riyadh/...) have negligible US collision and were the
// gaps that let non-US "City (Remote)" postings through.
const NON_US_COUNTRIES = /\b(india|china|japan|korea|singapore|israel|germany|france|spain|portugal|italy|netherlands|amsterdam|poland|romania|bulgaria|ukraine|russia|uk|united\s*kingdom|london|manchester|dublin|ireland|sweden|norway|finland|denmark|switzerland|austria|belgium|greece|turkey|mexico|brazil|argentina|colombia|chile|peru|canada|toronto|vancouver|montreal|ottawa|australia|sydney|melbourne|new\s*zealand|south\s*africa|nigeria|kenya|egypt|dubai|uae|saudi\s*arabia|riyadh|jeddah|doha|qatar|bahrain|kuwait|cyprus|gibraltar|luxembourg|malta|andorra|monaco|liechtenstein|iceland|reykjavik|latam|latin\s*america|czech|hungary|slovakia|slovenia|croatia|estonia|latvia|lithuania|taiwan|thailand|vietnam|philippines|indonesia|malaysia|pakistan|bangladesh|sri\s*lanka|serbia|bosnia|moldova|macedonia|montenegro|albania|georgia\s*country|armenia|azerbaijan|kazakhstan|uzbekistan|belgrade|sofia|bucharest|tallinn|warsaw|krakow|helsinki|oslo|stockholm|copenhagen|zurich|geneva|vienna|prague|budapest|bratislava|athens|tel\s*aviv|bangalore|mumbai|hyderabad|shanghai|beijing|seoul|taipei|hcmc|ho\s*chi\s*minh|hanoi|jakarta|manila|bangkok|kuala\s*lumpur|mexico\s*city|são\s*paulo|sao\s*paulo|buenos\s*aires|bogota|santiago)\b/i;
...
function parseLocation(raw) {
  ...
  // "City, Region" -> onsite. The region word identifies the country: a US
  // state code/name is US-onsite; any other region (e.g. "Maharashtra",
  // "Ontario", "England") is a FOREIGN onsite location -> us=false. This is
  // what stops a "Pune, Maharashtra" / On-Site role leaking through as
  // 'unknown' (the old regex only matched a 2-letter US state code).
  const cityRegion = remote === 'unknown'
    ? loc.match(/\b[A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+)*\s*,\s*([A-Za-z]{2,})\b/)
    : null;
  if (cityRegion) {
    remote = 'onsite';
    const region = cityRegion[1];
    if (STATE_CODES.has(region.toUpperCase()) || US_STATE_NAMES.has(region.toLowerCase())) us = true;
    else if (!US_PATTERNS.test(loc)) us = false; // foreign onsite
  }
  ...
  // Guard `us !== true`: an explicit US-state-coded city (set above, e.g.
  // "Paris, TX" -> us=true) must NOT be overridden by a foreign-name token
  // matching the same city stem. Only override when US-ness is unestablished.
  if ((isNonUsName || isNonUsCode) && !isUs && us !== true) us = false;
  else if (isUs && us === null) us = true;
  else if (us === null) {
