...
// ── Signal patterns against the candidate's portfolio (reference.md A1-A12) ───
...
const TITLE_INCLUDE = [
  ...
  /\bml research/i,
  /\bai engineer/i,
  /\bapplied (scientist|ml|ai|machine)/i,
  /\bresearch engineer/i,
  /\bdata scien(tist|ce)/i,
  /\bmlops\b/i,
  /\bllm\b/i,
  /\bgenai\b/i,
  /\bdeep learning\b/i,
  /\binference\b/i,
  /\bmember of technical staff/i,
  /\bnlp\b/i,
];

function isRoleMatch(title) {
  if (TITLE_EXCLUDE.some(re => re.test(title))) return false;
  return TITLE_INCLUDE.some(re => re.test(title));
}

// ── Quick score (no JD required) ────────────────────────────
...
function quickScore(posting) {
  ...
  const corpus = `${posting.title} ${posting.department || ''}`;
  let raw = 0;
  const archetypeHits = {};
  ...
  for (const sig of PORTFOLIO_SIGNALS) {
    if (sig.pattern.test(corpus)) {
      raw += sig.weight;
      if (sig.archetype) archetypeHits[sig.archetype] = (archetypeHits[sig.archetype] || 0) + sig.weight;
    }
  }
  for (const sig of SENIORITY_BOOST) {
    if (sig.pattern.test(posting.title)) raw += sig.weight;
  }
  ...
  const topArchetype = Object.entries(archetypeHits).sort((a, b) => b[1] - a[1])[0];
  posting.quickScore = bucket;
  posting.archetype = topArchetype ? topArchetype[0] : 'Applied ML';
  posting.rankReason = `quick: raw=${raw.toFixed(2)} bucket=${bucket} archetype=${posting.archetype}`;
  return posting;
}
...
function fullScore(posting, opts = {}) {
  ...
  for (const sig of FULL_SIGNALS.strong) {
    if (sig.regex.test(jd)) { score += sig.points; reasons.push(`+${sig.points} ${sig.why}`); }
  }
  for (const sig of FULL_SIGNALS.preferred) {
    if (sig.regex.test(jd)) { score += sig.points; reasons.push(`+${sig.points} ${sig.why}`); }
  }
  for (const sig of FULL_SIGNALS.negative) {
    if (sig.regex.test(jd)) { score += sig.points; reasons.push(`${sig.points} ${sig.why}`); }
  }
  ...
  // Gate-pass calibration (Required Skills proxy via JD match density; Experience via title)
  const hasSeniorTitle = /\b(senior|staff|principal|lead)\b/i.test(posting.title);
  posting.gatePass = {
    gate1: score >= 55,
    gate2: hasSeniorTitle || /\b(3\+|5\+|[3-9]\s*years|\bmid\b)/i.test(jd),
  };
